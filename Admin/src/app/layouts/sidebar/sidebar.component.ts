...
export class SidebarComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit() {
   ...
   filterMenuItems(menu: MenuItem[], role: string): MenuItem[] {
    return menu.filter(item => 
      !item.roles || item.roles.includes(role) // Show item if no roles specified OR user has the right role
    ).map(item => ({
      ...item,
  ...
  initialize(): void {
    ...
    this.menuItems = this.filterMenuItems(MENU, this.userRole[0]);
